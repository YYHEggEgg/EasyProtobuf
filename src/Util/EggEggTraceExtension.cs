...
public static class LoggerChannelTraceExtensions
{
    ...
    private static void Initialize()
    {
        lock (_exception_lck)
        {
            ...
            try
            {
                _tracelog = new BaseLogger(inheritingLoggerConfig, ErrTraceLogFileIdentifier);
            }
            catch (ArgumentException aex)
            {
                // https://github.com/YYHEggEgg/csharp-logger/blob/main/CSharp-Logger/LogFileStream.cs#L233
                if (aex.ParamName != "fileStreamName") throw;
